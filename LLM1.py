...
def query_context(question, llm_data):
    """
    Queries the LLM data to find the most relevant context based on the question.
    Uses cosine similarity to match the question's embedding with the context.
    """
    question_embedding = process_text(question)

    # Calculate cosine similarity between question and each text in the LLM
    similarities = []
    for text in llm_data["text"]:
        context_embedding = process_text(text)
        similarity = 1 - cosine(question_embedding, context_embedding)
        similarities.append(similarity)

    # Find the most relevant context (max similarity)
    most_relevant_idx = np.argmax(similarities)
    return llm_data["text"][most_relevant_idx]
...
#     """
#     Endpoint to answer a question based on the updated LLM.
#     """
    # Load all knowledge
    llm_data = load_llm_data()

    # Query the most relevant context from the LLM
    relevant_context = query_context(question, llm_data)

    # Generate a response in markdown
    markdown_response = generate_markdown_response(question, relevant_context)
